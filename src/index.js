...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    const scoreElement = document.querySelector("#schet");
    if (scoreElement) {
        scoreElement.innerHTML = snake.bodyLenght;
    }
  },
};
...
const snake = {
   ...
   */
  makeStep() {
    // Сохраняем направление, чтобы нельзя было развернуться на 180
...
const game = {
  ...
  getRandomFreeCoordinates() {
    // Бесконечный цикл пока не найдем свободную точку
    while (true) {
      const point = {
      ...
      // Проверяем если змейки тут нет и еды тут нет возвращаем точку
  ...
  canSetDirection(direction) {
    const lastDir = this.snake.getLastStepDirection();
    
    // Нельзя поворачивать на 180 градусов
    if (direction === 'up' && lastDir === 'down') return false;
    if (direction === 'down' && lastDir === 'up') return false;
    if (direction === 'left' && lastDir === 'right') return false;
    if (direction === 'right' && lastDir === 'left') return false;
  ...
  canMakeStep() {
    ...
    // Проверяем только самопересечение, стены проходятся насквозь в getNextStepHeadPoint
    return !this.snake.isOnPoint(nextHead);
  },
};
