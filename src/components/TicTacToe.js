export const TicTacToe = {
    ...
    this.boxes.forEach(box => {
      box.addEventListener('click', event => {
        ...
        if (this.checkForWin()) {
          ...
          // Добавил вывод победы
          setTimeout(() => {
             alert('Победил ' + this.getCurrentTurnValue())
          }, 10)
          
          return // Прерываем выполнение, чтобы не сработало условие ничьей
        }
   ...
   */
  checkHasEmptyBlocks() {
    // Проходим по всей матрице и ищем хоть один null
   ...
   */
  restartGame() {
    // 1. Сбрасываем переменные состояния
    ...
    // 3. Очищаем поле (DOM)
    this.boxes.forEach(box => {
      this.setBlockDom(box, true); // true означает "очистить"
    });

    // 4. Обновляем надпись "Сейчас ходит: X"
    if (this.onMove) {
      this.onMove(this.isXTurn);
    }
  },
